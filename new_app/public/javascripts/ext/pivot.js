  ...
  callWithJQuery(function($) {
    ...
    renderers = {
      ...
      "Col Heatmap": function(data, opts) {
		...
		//============================Change starts 27th july 2018 by somil==============
      "Download CSV" : function(data,opts) {
		  return csvexport(data, opts);
	  }
		//==========================Chenge ends================
    };
    locales = {
     ...
     */
	  ...
	  // Change added on 18th july 2018 by somil
	  //Function to download csv of chart data

	  csvexport = function(pivotData, opts) {
		  ...
		  result = [];
		  row = [];
		  let finalRow = []; // declared a final row
		  for (i = 0, len = rowAttrs.length; i < len; i++) {
			  rowAttr = rowAttrs[i];
			  if (typeof rowAttr === 'string' || rowAttr instanceof String)	
				  row.push(rowAttr.replace(/,/g,'-'));
			  else
				  row.push(rowAttr);
			  if(i === len-1) { // check for the end of row headers
				  finalRow.push('TOTAL');
			  } else {
				  finalRow.push('');
			  }
		  ...
		  if (colKeys.length === 1 && colKeys[0].length === 0) {
			  ...
			  for (j = 0, len1 = colKeys.length; j < len1; j++) {
				  colKey = colKeys[j];
				  var colKeyNew = [];
				  for (var count = 0; count < colKey.length; count++) {
					  var cK = colKey[count];
					  colKeyNew.push(cK.toString ().replace(/,/g,'-'));
				  }
				  row.push(colKeyNew.join("-"));
			  }
		  }
		  row.push('Total');
		  result.push(row);
		  for (k = 0, len2 = rowKeys.length; k < len2; k++) {
			  ...
			  for (l = 0, len3 = rowKey.length; l < len3; l++) {
				  r = rowKey[l];
				  row.push(r.toString ().replace(/,/g,'-'));
			  }
			  for (m = 0, len4 = colKeys.length; m < len4; m++) {
				  colKey = colKeys[m];
				  agg = pivotData.getAggregator(rowKey, colKey);
				  if (agg.value() != null) {
					  row.push(agg.value().toFixed(6) );
				  } else {
					  row.push("");
				  }
			  }
			  row.push(pivotData.getAggregator(rowKey,[]).value().toFixed(6)); // row totals
			  result.push(row);
		  }
		  for (let m = 0, len5 = colKeys.length; m < len5; m++) { // extra loop to add col totals
			  colKey = colKeys[m];

			  finalRow.push(pivotData.getAggregator([],colKey).value().toFixed(6));
		  }
		  finalRow.push(pivotData.getAggregator([],[]).value().toFixed(6)); // at last add the col totals row
		  ...
		  var csvData = new Blob([content], { type: "application/csv;charset=utf-8;" });
    ...
    $.fn.pivot = function(input, inputOpts, locale) {
      ...
      opts.cols = inputOpts.cols; opts.rows = inputOpts.rows;
      result = null;
      try {
